...
class VishamCEnv(Env):
    ...
    def _step(self, action):
        ...
        if np.array([np.greater(self.state, 1), np.less(self.state, -1)]).any():   # If agent crosses boundary end
            self.reward = -(0.5*(self.state[0]**2)+0.5*10*(self.state[1]**2))      # episode and give negative reward
            self.reset()
            ...
            self.reward = -(0.5*(self.state[0]**2)+0.5*10*(self.state[1]**2))
            self.done = False
